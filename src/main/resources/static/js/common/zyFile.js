...
var ZYFILE = {
	  ...
	  /* 提供给外部的接口 */
		...
		funUploadFiles : function(){
			...
			// 遍历所有文件  ，在调用单个文件上传的方法
            var fileName="";
            for(var i=0;i<self.uploadFile.length;i++){
                fileName+=self.uploadFile[i].name+";";
                //控制图片上传数量
				if(i>5)
				{
                    return msg("图片数量不可大于5");
                }
            }
           ...
           /* if(fileName.length!=0){
                $.ajax({
                    type: 'post',
                    async: true,
                    url: '/userperce/getSitePhoto/inesrtDataBase',
                    data: {orderNum: orderNum,fileName:fileName},
		...
		funUploadFile : function(file){
		   ...
		   // window.parent.getAreaData();
			//window.parent.setDistrictDetail(orderNum, 'upload');
...
*/		    
		...
		// 上传多个文件
		funConfirmFiles : function(){
			
			var orderNum =  $("#orderNumImage").val();
			var self = this;  // 在each中this指向没个v  所以先将this保留
			var haveImgCount = 0; //数据库存在的照片数
			var result = [];
            $.ajax({
                type: 'post',
                async: false,
                url: '/userperce/getSitePhoto/getPhotoUrlList',
                data: {orderNum: orderNum},
                success: function (res) {
                	result = res;
                	haveImgCount = res.length;
                }
            });
            
            for(var i=0; i< self.uploadFile.length; i++){
            	var name = self.uploadFile[i].name;
            	for(var j=0; j<result.length;j++){
            		if(result[j].indexOf(name) != -1){
                		return  msg("存在相同文件名");
                	}
            	}
            ...
            //控制图片上传数量
			if(self.uploadFile.length + haveImgCount > 5){
				if(haveImgCount == 0){
					return msg("图片数量不能超过5张");
				}else{
					return msg("已经存在"+haveImgCount+"张照片,只能上传"+(5-haveImgCount)+"张照片");
				}
            }else{
            	//隐藏图片上传窗口
            	$("#uploadImage").addClass("hide");
            }
			selectPhoneCount = self.uploadFile.length;
			 var ss = "";
             ss+='<span>已选择：'+self.uploadFile.length+'张图片</span>';
             $('#selectphotos').html(ss);
		},
		// 初始化
		init : function(){  // 初始化方法，在此给选择、上传按钮绑定事件
			...
			if(self.fileInput){
				// 绑定change事件
                    $("#uploadImage").on('change','#fileImage',function (e) {
                    var $inputfile = '<input id="fileImage" type="file" size="30" name="fileselect[]" multiple="">';
                    //替换原来的input file ,避免在删除这个文件后，重复选时不能选
                    $(this).replaceWith($inputfile);
                    self.fileInput=$inputfile;
					self.funGetFiles(e);
				});
            }
			...
			// 图片上传确认
			if(self.fileConfirm){
				this.fileConfirm.addEventListener("click", function(e) {
					isClickUpImg = 1;
					self.funConfirmFiles(e); 
				}, false);	
			}
		}
};
