(function (global, factory) {
    ...
    DataSet.prototype.add = function (data, senderId) {
       ...
       // this._dataCache = JSON.parse(JSON.stringify(this._data)); YUAN注释
    };
        ...
        draw: function draw(context, dataSet, options) {
            ...
            if (options.bigData) {
                ...
                if (type == 'Point' || type == 'Polygon' || type == 'MultiPolygon') {
                    ...
                    if (item.lineDash||options.lineDash) {
                        context.setLineDash(item.lineDash||options.lineDash);
                    }
                ...
                for (var i = 0, len = data.length; i < len; i++) {
                    ...
                    if (item.fillStyle ||options.fillStyle|| item._fillStyle) {
                        context.fillStyle = item.fillStyle ||options.fillStyle|| item._fillStyle;
                    }

                    if (item.strokeStyle || options.strokeStyle||item._strokeStyle) {
                        context.strokeStyle = item.strokeStyle ||options.strokeStyle|| item._strokeStyle;
                    }
                    ...
                    if (type == 'Point' || type == 'Polygon' || type == 'MultiPolygon') {
                        ...
                        if (item.lineWidth||options.lineWidth || item._lineWidth) {
                            context.lineWidth = item.lineWidth ||options.lineWidth|| item._lineWidth;
                        }
                        context.stroke();
                    }
    ...
    var MapHelper = function () {
        ...
        createClass(MapHelper, [{
            ...
            value: function addLayer(datas, options) {
                if (this.type == 'baidu') {
                    return new mgv.baiduMapLayer(this.map, dataSet, options);
                }
            }
        }, {
            key: 'getMap',
...
//判断2个矩形是否有重叠部分
        ...
        draw: function draw(context, dataSet, options) {
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                ...
                if (data[i].geometry) {
                    ...
                    var icon = data[i].icon || options.icon;//icon
                    var coordinates = data[i].geometry._coordinates || data[i].geometry.coordinates;
                    var x = coordinates[0];
                    var y = coordinates[1];
                    if (deg) {
        ...
        createClass(BaseLayer, [{
            ...
            value: function isPointInPath(context, pixel) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (geoType.indexOf('LineString') >=0) {
            ...
            value: function clickEvent(pixel, e) {
                ...
                if (dataItem) {
                    this.options.methods.click(dataItem, e,this.canvasLayer.lid);
                } else {
            ...
            value: function mousemoveEvent(pixel, e) {
                ...
                var dataItem = this.isPointInPath(this.getContext(), pixel);
                if (dataItem) {
                    this.options.methods.mousemove(dataItem, e,this.canvasLayer.lid);
                } else {
            ...
            {//****yuan*****
                key: "dbclickEvent",
                value: function dbclickEvent(pixel, e) {

                    var dataItem = this.isPointInPath(this.getContext(), pixel,this.canvasLayer.lid);

                    if (dataItem) {
                        this.options.methods.dbclick(dataItem, e);
                    } else {
                        this.options.methods.dbclick(null, e);
                    }
                }
            }, {//****yuan*****
                key: "rightclickEvent",
                value: function rightclickEvent(pixel, e) {

                    var dataItem = this.isPointInPath(this.getContext(), pixel);
                    if (dataItem) {
                        this.options.methods.rightclick(dataItem, e,this.canvasLayer.lid);
                    } else {
                        this.options.methods.rightclick(null, e,null);
                    }
                }
            }, {
            key: "update",
    ...
    var Layer = function (_BaseLayer) {
        ...
        function Layer(map, dataSet, options) {
            ...
            _this.dbclickEvent = _this.dbclickEvent.bind(_this);
            _this.rightclickEvent = _this.rightclickEvent.bind(_this);
            ...
            var canvasLayer = _this.canvasLayer = new CanvasLayer({
                ...
                lyrcode: options.lyrcode,//****yuan***** 要改动
                loadend: options.loadend,//****yuan*****
                update: function update() {
                    self._canvasUpdate();
                },
                get: function get() {//****yuan*****
        ...
        createClass(Layer, [{
            ...
            {
                key: "dbclickEvent",
                value: function dbclickEvent(e) {
                    var pixel = e.pixel;
                    get(Layer.prototype.__proto__ || Object.getPrototypeOf(Layer.prototype), "dbclickEvent", this).call(this, pixel, e);
                }
            }//****yuan*****
            , {
                key: "rightclickEvent",
                value: function rightclickEvent(e) {
                    var pixel = e.pixel;
                    get(Layer.prototype.__proto__ || Object.getPrototypeOf(Layer.prototype), "rightclickEvent", this).call(this, pixel, e);
                }
            }
        , {
            key: "bindEvent",
            value: function bindEvent(e) {
                ...
                if (this.options.methods) {
                    ...
                    if (this.options.methods.dbclick) {//****yuan*****
                        map.setDefaultCursor("default");
                        map.addEventListener('dblclick', this.dbclickEvent);
                    }
                    if (this.options.methods.rightclick) {//****yuan*****
                        map.setDefaultCursor("default");
            ...
            value: function unbindEvent(e) {
                ...
                if (this.options.methods) {
                    ...
                    if (this.options.methods.rightclick) {//****yuan*****
                        map.removeEventListener('rightclick', this.rightclickEvent);
                    }
                    if (this.options.methods.dbclick) {//****yuan*****
                        map.removeEventListener('dblclick', this.dbclickEvent);
                    }
                }
            }
            ...
            value: function _canvasUpdate(time) {
                if (!this.canvasLayer||this.dataSet._data.length==0) {
                    return;
                }
                var self = this;
                ...
                if (self.options.unit == 'm') {
                    if (self.options.size) {
                        ...
                        self.options._size = self.options._size + self.options._size / 10;//yuan 修正
                    }
                    if (self.options.width) {
                        self.options._width = self.options.width / zoomUnit;
                        self.options._width = self.options._width + self.options._width / 10;//yuan 修正
                    }
                    if (self.options.height) {
                        self.options._height = self.options.height / zoomUnit;
                        self.options._height = self.options._height + self.options._height / 10;//yuan 修正
                    }
                } else {
        ...
        createClass(Layer, [{
            ...
            value: function bindEvent(e) {
                ...
                if (this.options.methods) {
                    ...
                    if (this.options.methods.rightclick) {//****yuan*****
                        map.addListener('rightclick', this.rightclickEvent);
                    }
                    if (this.options.methods.dbclick) {//****yuan*****
                        map.setDefaultCursor("default");
                        map.addListener('dblclick', this.dbclickEvent);
                    }
                }
            }
        }, {
            ...
            value: function unbindEvent(e) {
                ...
                if (this.options.methods) {
                    ...
                    if (this.options.methods.rightclick) {//****yuan*****
                        map.removeListener('rightclick', this.rightclickEvent);
                    }
                    if (this.options.methods.dbclick) {//****yuan*****
                        map.removeListener('dblclick', this.dbclickEvent);
                    }
                }
            }
        }, {
...
// import * as maptalks from 'maptalks';
    ...
    if (typeof maptalks !== 'undefined') {
        Layer$4 = function (_maptalks$Layer) {
            ...
            createClass(Layer, [{
                ...
                {
                    key: "rightclickEvent",//****yuan*****
                    value: function rightclickEvent(pixel, e) {

                        var dataItem = this.isPointInPath(this.getContext(), pixel);
                        if (dataItem) {
                            this.options.methods.rightclick(dataItem, e);
                        } else {
                            this.options.methods.rightclick(null, e);
                        }
                    }
                }
                , {
                    ...
                    value: function getEvents() {
                        return {
                            ...
                            'rightclick': this.rightclickEvent,//****yuan*****
                            'dbclick': this.dbclickEvent//****yuan*****
                        };
                    }
                }, {
                    key: "init",
        ...
        var LayerRenderer = function (_maptalks$renderer$Ca) {
            ...
            createClass(LayerRenderer, [{
                ...
                value: function _bindToMapv() {
                    //some bindings needed by mgv baselayer
                    var base = this.layer.baseLayer;
    ...
    /**
     * mgv for openlayers (https://openlayers.org)
     * @author sakitam-fdd - https://github.com/sakitam-fdd
     */
    ...
    var $2=function (map,data,option) {
        if(!map||!data||data.length==0||!option)
            return;
        var d=new DataSet(data);
        var opt={};
        // for(var p in option._option) {
        //     opt[p] = option._option[p];
        // }
        $.extend(opt,option._option);
        opt.lyrcode=option._lc;
        var nm=new Layer(map,d,opt);
    }
    /**
     * @author kyle / http://nikai.us/
     */
    ...
    var g= {
         Layer: function (map, lyrcode) {
            if (!map)
                return;
            var overlay = map.getOverlays();
            for (var v in overlay) {
                if (overlay[v].lid && overlay[v].lid == lyrcode) {
                    return overlay[v].options.get();
                }
            }
        },
         Data: function (map, lyrcode) {
            if (!map)
                return;
            var ly = this.getLayer(map, lyrcode);
            if (ly) {
                return ly.dataSet._data;
            }
        }
    }

    // var getLayers = function (map, mapcode) {
    //     if (!map)
    //         return;
    //     var overlay = map.getOverlays();
    //     for (var v in overlay) {
    //         if (overlay[v].options && overlay[v].options.lyrcode) {
    //             if (overlay[v].options.lyrcode === mapcode) {
    //                 return overlay[v].options.get();
    //             }
    //         }
    //     }
    ...
    /**
     *
     * @param city city/citycode
     * @param callback
     */
    var $json=function(city,callback) {
        var json;
        if (city == '丽江') {
            json = '../../spmv/data/regionJson/Lijiang.json';
        }
        if (json) {
            var data = [];
            $.get(json, function (rs) {
                $.each(rs.features, function () {
                    delete  this.geometry.encodeOffsets;
                    data.push({
                        id: this.id,
                        geometry: this.geometry,
                        name: this.properties.name,
                        cp: this.properties.cp
                    })
                })
